...
{
    public class NamespaceGetCommand : Command<NamespaceGetSettings>
    {
        public override int Execute(CommandContext context, NamespaceGetSettings settings, CancellationToken cancellationToken)
        {
            ...
            var result = vaultService.GetSecret(vault, ns, key);
            if (result.IsFailure)
            {
            ...
            AnsiConsole.MarkupLine($"[green]-[/] Retrieved configuration: [blue]{ns}::{key}[/] = [blue]{result.Data!}[/]");
            return 0;
        }
    }
}
