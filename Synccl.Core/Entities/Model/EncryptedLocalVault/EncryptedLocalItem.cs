...
namespace Synccl.Core.Entities.Model.EncryptedLocalVault
{
    public sealed class EncryptedLocalItem
    {
        ...
        public EncryptedBlob Payload { get; private set; }
            ...
            IEnumerable<KeyWrap> wrappedItemKeys)
        {
            ...
            Id = id;
            Key = key;
            Payload = EncryptedBlob.From(payload);
        ...
        public void SetPayload(EncryptedBlob newPayload)
        {
            if (newPayload is null)
                throw new ArgumentNullException(nameof(newPayload));
            Payload = EncryptedBlob.From(newPayload);
        }
